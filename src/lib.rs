...
mod tests {
    ...
    #[test] fn test_anti_correlated_high_mi() { let x = vec![1,-1,1,-1,1,-1]; let y = vec![-1,1,-1,1,-1,1]; let mi = mutual_information(&x, &y); assert!(mi > 0.5, "MI of anti-correlated should be high: {}", mi); }
    #[test] fn test_mi_symmetric() { let x = vec![1,0,-1]; let y = vec![-1,0,1]; assert!((mutual_information(&x, &y) - mutual_information(&y, &x)).abs() < 0.001); }
    #[test] fn test_entropy_uniform() { let probs = vec![1.0/3.0, 1.0/3.0, 1.0/3.0]; let h = entropy(&probs); assert!((h - 1.585).abs() < 0.01, "entropy was {:.3}", h); }
